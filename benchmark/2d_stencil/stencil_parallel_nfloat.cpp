                ...
                const std::size_t& steps, const std::size_t& chunk_size)
{
    ...
    // using executor_type = hpx::parallel::execution::parallel_executor_aggregated;
    using data_type = hpx::compute::vector<_Tx, allocator_type>;
    // using data_type = hpx::compute::vector<_Tx>;
    ...
    for (std::size_t t = 0; t < steps; ++t)
    {
        hpx::parallel::for_each(
                // policy.with(auto_chunk_size( chunk_size )),
                policy,
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    ...
    // std::cout << "Total thread execution time: " << timer << std::endl;
    // timer = 0;
    // stencil_main<vfloat>(Nx, Ny, steps, chunk_size);
    // std::cout << "Total thread execution time: " << timer << std::endl;

    // timer = 0;
    // stencil_main<dfloat>(Nx, Ny, steps, chunk_size);
...
int main(int argc, char* argv[])
{
    ...
    // Make sure to steal work from same NUMA domain thread
    std::vector<std::string> const cfg = {
#if defined(__x86_64__)
        "hpx.numa_sensitive=2"
#elif defined(__aarch64__)
        "hpx.numa_sensitive=4"
#endif
    };
